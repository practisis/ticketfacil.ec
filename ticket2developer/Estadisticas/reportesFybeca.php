...
<div style="">
	<div style="background-color:#171A1B; ">
		<center><img src = 'imagenes/loader.gif' id = 'loaderGif'/></center>
		<div style="border: 2px solid #00AEEF;" id = 'contenedorConciertos'>
			
			<div style="background-color:#00ADEF;position:relative; padding:10px; text-align:center; color:#fff; font-size:18px;width:100%;" id='contRepAjax'> 
				
				<div >
					<table class='table' style='font-size:12px;border: 1px solid #fff;width:100%;display:none;' id = 'contieneLosConciertos'>
						<tr>
							<th colspan = '7' >
								VENTAS TODOS LOS CONCIERTOS
							</th>
						</tr>
						<tr>
					...
					?>
						<tr style = "display:none;">
								...
								<table width = "100%">
									<tr>
										<td style = "border-right:1px solid #fff; " >
											<?php echo $globalEfectivo;?>
										</td>
										<td>
											<?php echo $globalTarjeta;?>
										</td>
									</tr>
								</table>
							...
							<td>
								<?php echo $globalNumeroTick; ?>
							</td>
							
							<td>
								<?php echo $globalValorRecibido; ?>
							</td>
							<td>
								<span id = "sumaComRet">
				...
				<!--<iframe src = 'http://ticketfacil.ec/ticket2/Estadisticas/reporteFybecaWeb.php'  class = 'iframes' frameBorder="0" id = 'iframe1'></iframe>
				<br/>
				
				<iframe src = 'http://ticketfacil.ec/ticket2/Estadisticas/reporteFybecaCadena.php' class = 'iframes' frameBorder="0" id = 'iframe2' ></iframe>
				<br/>
				
				<iframe src = 'http://ticketfacil.ec/ticket2/Estadisticas/reporteFybecaCadenaTF.php' class = 'iframes' frameBorder="0" id = 'iframe3' ></iframe>
				-->
				<iframe src = 'http://ticketfacil.ec/ticket2/Estadisticas/ajax/cadenaTF.php?idConcierto=38' style = 'width: 1000px;height: 450px; <?php echo $display2;?>'  frameBorder="0" ></iframe>
				<iframe src = 'http://ticketfacil.ec/ticket2/Estadisticas/ajax/cadenaTF2.php?idConcierto=38' style = 'width: 1000px;height: 450px;  <?php echo $display1;?>'  frameBorder="0" ></iframe>
				
				<br/>
				<div id='recibeLocalidades'></div>
			</div>
		</div>
	</div>
</div>
<script>
	$( document ).ready(function() {
		$('#contenedorConciertos').fadeOut('fast');
		setTimeout(function(){
			//alert("Boom!");
			var sumaComRet = 0;
			$('.numeroConcierto').each(function(){
				var numCon = $(this).attr('numeroConc');
				var comisionesRetencionesTarjetaWeb_ = $("#iframe1").contents().find('.comisionesRetencionesTarjetaWeb_'+numCon).val();
				var comisionesRetencionesTarjetaCadena_ = $("#iframe2").contents().find('.comisionesRetencionesTarjetaCadena_'+numCon).val();
				var comisionesRetencionesTarjetaTF_ = $("#iframe3").contents().find('.comisionesRetencionesTarjetaTF_'+numCon).val();
				console.log(comisionesRetencionesTarjetaWeb_ + ' <> ' + comisionesRetencionesTarjetaCadena_ + ' <> ' + comisionesRetencionesTarjetaTF_ + ' <> ');
				//parseFloat(comisionesRetencionesTarjetaWeb_) + aqui 
				var sumaComisiones = ( parseFloat(comisionesRetencionesTarjetaCadena_) + parseFloat(comisionesRetencionesTarjetaTF_)).toFixed(2);
				sumaComRet += parseFloat(sumaComisiones).toFixed(2);
				var sumaValorConcierto_ = $('.sumaValorConcierto_'+numCon).val();
				
				var totalPagar_ = (parseFloat(sumaValorConcierto_) - parseFloat(sumaComisiones));
				
				$('.totalPagar_'+numCon).html(totalPagar_);
				
				console.log(sumaComisiones +"  "+ sumaComRet);
				$('.sumaComisiones_'+numCon).html(sumaComisiones);
				$('#contenedorConciertos').css('display','block');
				$('#loaderGif').css('display','none');
			});
			$("#sumaComRet").html(sumaComRet);
		}, 3000);
	});
	function enviaConcierto(idConcierto){
		
		$.post("Estadisticas/ajax/localidadConciertoFybeca.php",{ 
			idConcierto : idConcierto
		}).done(function(data){
			var posisionForm = ($('#recibeLocalidades').position().top + 300);
			$( "html, body " ).animate({
	...
	$( document ).ready(function() {
		console.log( "ready!" );
		var ancho = $('#contieneLosConciertos').width();
		var alto = $('#contieneLosConciertos').height();
		console.log('ancho : '+ ancho + ' alto : ' + alto );
		$('.iframes').css('width',ancho);
		$('.iframes').css('height',alto);
		$('.ifr22').css('width',ancho);
		$('.ifr22').css('height',alto);
		$('.ifr').css('height','440px');
