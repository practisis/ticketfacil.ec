	...
	<table class='table' style='font-size:12px;border: 1px solid #fff;width:100%;color:#fff;'>
		<tr>
			<th colspan = '7' >
				VENTAS PAGINA WEB TODOS LOS CONCIERTOS
			</th>
		</tr>
		<tr>
		...
		<tr>
					...
					<tr>
						...
						<td width='50%'  style='font-size:12px;'>
							...
							//	echo number_format(($sumValTarTer),2)."<br>";
								...
								echo number_format(($sumaTarjeta),2);//."ghgh";
			...
			<td style='font-size:12px;border: 1px solid #fff'>
				<?php
					...
					$sqlComWeb2 ='SELECT * FROM `comi_ret` WHERE `id_con` =  "'.$rowCon['idConcierto'].'" and detalle = "PAGINA WEB" ';
					...
					$comisionTarjeta = (($recaudacionTarjetas * $comi_tar2)/100);
					
					//$retencionTarjeta2 = number_format(((($recaudacionTarjetas)*($ret_renta2/100))+((($recaudacionTarjetas) - (($recaudacionTarjetas) / 1.14))*($ret_iva2/100))),2);
					$retencionTarjeta2 = number_format(((($sumaTarjeta / 1.14)*($ret_renta2/100))+(($sumaTarjeta - ($sumaTarjeta / 1.14))*($ret_iva2/100))),2);
					// echo $sumaTarjeta."<<suma T>><br>";
					// echo $comisionTarjeta."<<comision T>><br/>";
					// echo $retencionTarjeta2."<<retencion T>><br/><hr>";
					//echo "(".$sumaTarjeta."*".$ret_renta2."/100)+(".$sumaTarjeta."-".$sumaTarjeta."/ 1.14) * (".$ret_iva2." /100)<br/>";
					$comisionesRetencionesTarjeta = ($comisionTarjeta + $retencionTarjeta2);
					echo number_format(($comisionesRetencionesTarjeta),2);
					echo "<input type = 'hidden' class = 'comisionesRetencionesTarjetaWeb_".$rowCon['idConcierto']."' value = '".number_format(($comisionesRetencionesTarjeta),2)."'/>";
