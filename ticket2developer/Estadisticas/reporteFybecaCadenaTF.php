	...
	<table class='table' style='font-size:12px;border: 1px solid #fff;width:100%;'>
		<tr>
			<th colspan = '7' >
				VENTAS CADENA TICKET FACIL TODOS LOS CONCIERTOS
			</th>
		</tr>
		<tr>
	...
	?>
		<tr>
			...
			<td style='font-size:12px;border: 1px solid #fff'>
				<?php
					$sqlComWeb1 ='SELECT * FROM `comi_ret` WHERE `id_con` =  "'.$rowCon['idConcierto'].'" and detalle = "PUNTOS TICKET FACIL" ';
					...
					$comisionTarjeta = (($recaudacionTarjetas * $comi_tar1)/100);
					
					$retencionTarjeta2 = number_format(((($recaudacionTarjetas / 1.14)*($ret_renta1/100))+((($recaudacionTarjetas) - (($recaudacionTarjetas) / 1.14))*($ret_iva1/100))),2);
					
					// echo "comision : ".number_format(($comisionTarjeta),2)."<br>";
					// echo "retencion : ".number_format(($retencionTarjeta2),2)."<br>";
					$comisionesRetencionesTarjeta = ($comisionTarjeta + $retencionTarjeta2);
					echo number_format(($comisionesRetencionesTarjeta),2).'rrr';
					echo "<input type = 'hidden'  class = 'comisionesRetencionesTarjetaTF_".$rowCon['idConcierto']."' value = '".number_format(($comisionesRetencionesTarjeta),2)."' />";
				?>
			</td>
			<td style='font-size:12px;border: 1px solid #fff'>
				<?php
