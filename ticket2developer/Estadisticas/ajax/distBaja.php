...
<?php
	...
	while($row = mysql_fetch_array($res)){
		...
		$sqlB = 'SELECT count(idBoleto) as vendidos_normal FROM `Boleto` WHERE `idCon` = "'.$idcon.'0000" AND `idLocB` = "'.$row['idLocalidad'].'" AND `tercera` = 0 '.$query.' ORDER BY `idLocB` DESC ';
		...
		$sqlB2 = 'SELECT count(idBoleto) as vendidos_todos FROM `Boleto` WHERE  `idLocB` = "'.$row['idLocalidad'].'" and tercera = 0 and id_desc != "'.$rowDD['id'].'"  ORDER BY `idLocB` DESC ';
...
?>
		<tr>
			...
			<td style = '<?php echo $display;?>'>
				...
				<?php 
					if($rowB['vendidos_normal'] == 0){
						$colorT = '';
					}else{
						$colorT = 'background-color: #000;text-align: center; color:green;font-weight: bold;font-size: 18px';
					}
					echo "<td style = '".$colorT."'>".$rowB['vendidos_normal']."</td>";
		...
		<tr>
			...
			<td style = '<?php echo $display;?>'>
				...
				<?php
					...
					if($rowB1['vendidos_descuento'] == 0){
						$colorT2 = '';
					}else{
						$colorT2='background-color: #000;text-align: center; color:green;font-weight: bold;font-size: 15px';
					}
					echo "<td style = '".$colorT2."' >".$rowB1['vendidos_descuento']."<td>";
...
?>
		<tr >
			...
			<td style = 'background-color: #000;text-align: center; color:green;font-weight: bold;font-size: 18px'>
