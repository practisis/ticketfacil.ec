...
if($numBol == 0){
	...
	echo '<input type="hidden" id="evento" value="'.$_GET['evento'].'" />';
...
<div style="background-color:#171A1B; border: 2px solid #00AEEF; margin:20px 20px 10px;">
					...
					?>
						<tr>
							...
							<td style = 'border:1px solid #fff;'>
								...
								<button id = 'boton_localidad_<?php echo $rowL['idLocalidad'];?>' type="button" class="btn btn-success btn-xs" onclick="asientos_mapa('<?php echo $rowL['idLocalidad'];?>','<?php echo $idcon;?>');">
									ABRIR LOCALIDAD <br/>
									<center><img src = 'http://ticketfacil.ec//ticket2/imagenes/ajax-loader2.gif' style = 'display:none;' id = 'load_localidad_<?php echo $rowL['idLocalidad'];?>' /></center>
								</button>
							</td>
						</tr>
					<?php
		...
		<div style="background-color:#00AEEF; margin:20px 30px 20px -32px; position: relative;">
			<div class="row">
				...
				<div class="col-lg-12" style="margin-top:5%;">
					<center><img src="<?php echo $r; ?>" class="img-responsive"/></center>
				</div>
			</div>
	...
	<div id="mostrar_mapa">
		...
		<div style="background-color:#272b2a; border:1px solid #00aeef; margin:0 125px; padding:60px 0; position:relative;">
			...
			<div id="img_localidad<?php echo $idbtnloc;?>" style="margin: 0 auto; width: 100%; display: none;">
				<center>
					<div id="divisiones_mapa<?php echo $idbtnloc;?>" style="width:auto; height:100px; background-image:url(<?php echo $ruta_img_local;?>); background-repeat:no-repeat; background-size:500px 100px;"></div>
				</center>
	...
	<div id = 'contieneNoNumerados' style = 'display:none;' >
		...
		<table class = 'table' style = 'width:80%;' align = 'center' id = 'contenedorVisualLocalidad'>
			<tr>
				<td>
		...
		for(var m = 1; m <= sinNumeracion; m++){
			...
			$('#seleccion').append('<tr class = "filaSinNumeracion_'+m+' asientosescogidos asientosok">\
										<td style="text_align:center; border:none;">\
											<center>\
												<font color="#000">\
													<input name="codigo[]" class="form-control added" style = "background-color:transparent;" value="'+localidadSinNum+'" readonly="readonly" size="5" type="text">\
										...
										<td>\
											<center><font color="#000"><input name="chair[]" class="form-control added" style = "background-color:transparent;" value="Asientos no Numerados" readonly="readonly" size="15" type="text"></font></center>\
										</td>\
										<td>\
											<center><font color="#000"><input name="des[]" class="form-control added" style = "background-color:transparent;" value="'+nomLoc+'" readonly="readonly" size="10" type="text"></font></center>\
										</td>\
										<td>\
											<center><font color="#000"><input name="num[]" class="form-control added" style = "background-color:transparent;" value="1" readonly="readonly" size="10" type="text"></font></center>\
										</td>\
										<td>\
											<center><font color="#000"><input name="pre[]" class="form-control added" style = "background-color:transparent;" value="'+precioLocalidadSinNum+'" readonly="readonly" size="10" type="text"></font></center>\
										</td>\
										<td>\
											<center><font color="#000"><input name="tot[]" class="form-control added" style = "background-color:transparent;" value="'+precioLocalidadSinNum+'" readonly="readonly" size="10" type="text"></font></center>\
										</td>\
										<td>\
			...
			datos += localidadSinNum +'|'+ precioLocalidadSinNum +'|'+ cantidad +'|'+ roww +'|'+ coll +'|'+ l +'|'+ nomLoc +'|'+ cantidad +'|'+ evento +'@';
		}
		
		$.post("distribuidor/eventos/creaSesionNoNumerados.php",{ 
			datos : datos
		}).done(function(data){
			// alert(data)			
		});
	...
	<form id="form" action="" method="post">
		<div id="ocultar_mapa">
			...
			<div style="height:auto; background-color:#EC1867; margin:50px -32px 10px 40%; padding-right:30px; color:#fff; font-size:18px; position:relative;">
				<div class="row" id = 'contieneBotonesComprar'>
					<div class="col-lg-5">
					...
					<div class="col-lg-5">
						<?php
							...
							// if($_SESSION['valida_ocupadas'] == 0){
						?>
							<button type="button" id="ok_2" class="btn_compra_online" style="margin-left:50px;display:none;"><img src="imagenes/mano_comprar.png" style="margin-left:-20px;"/><strong>+ Continuar</strong></button>
						<?php
							// }else{
						?>
							<button type="button" id="ok" class="btn_compra_online" style="margin-left:50px;display:none;"><img src="imagenes/mano_comprar.png" style="margin-left:-20px;"/><strong>+ Continuar</strong></button>
						<?php
							// }
						?>
					...
					<div id="choose_local">
						...
						<div class="row">
							...
							<div class="col-md-5">
								...
								<h3>
									<span class="label label-info form-control">
								...
								<?php }?>
								<input type="text" id="idLocalale" value="" />
							</div>
						</div>
					</div>
	...
	<script>
		function select_asientos_ale(){
			...
			$.post('distribuidor/eventos/saberdisp.php',{
				...
				alert(' disponibilidad de tickets :  '  +  data + ' , num tickets solicitados :  '  +  numticket);
				...
				}else if((parseInt(data) == 0) || (parseInt(data) < 0)){
...
<?php
	...
	if(isset($_REQUEST['idloc'])){
		$sqlLo = 'select * from Localidad where idLocalidad = "'.$_REQUEST['idloc'].'" ';
		$resLo = mysql_query($sqlLo) or die (mysql_error());
		$rowLo = mysql_fetch_array($resLo);
		echo '<input type = "hidden"  id = "strDescripcionL" value = "'.$rowLo['strDescripcionL'].'" />';
		echo '<input type = "hidden"  id = "doublePrecioL" value = "'.$rowLo['doublePrecioL'].'" />';
?>
			<script>
				asientos_mapa1(<?php echo $_REQUEST['idloc'];?>, <?php echo $_GET['evento'];?>);
				
				
				function asientos_mapa1(local, concierto){
					$('#contenedorVisualLocalidad').css('display','none');
				...
				$( document ).ready(function() {
					asignaSinNum1(<?php echo $_REQUEST['cantidad_aut']?> , <?php echo $_REQUEST['idloc'];?>);
				});
				
				
				
				function asignaSinNum1(sinNumeracion,id_loc){
					
					var evento = $('#evento').val();
					var con = $('#evento').val();
					//alert(sinNumeracion);
					// $('#seleccion').html('');
					var precioLocalidadSinNum = $('#doublePrecioL').val();
					var localidadSinNum = id_loc;
					var nomLoc = $('#strDescripcionL').val();
					var l = 0;
					var datos = '';
					var cantidad = 1;
					var roww = 'sin A';
					...
					var idlocal = id_loc;
					var numticket_2 = sinNumeracion;
					
					$.post("distribuidor/eventos/aleatorio_reconfigura.php",{ 
						con : con , idlocal : idlocal , numticket_2 : numticket_2
					}).done(function(data){
						$('#seleccion').html(data);		
					});
				...
				function animarLocal(){
					var videoHeight = $('#seleccion').height();
					// alert(videoHeight);
					$( "html, body " ).animate({
