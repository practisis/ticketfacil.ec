		...
		for($y = 1; $y <= $row['col']; $y++){
			if(in_array($y,$arr[$i][$y])){
				...
				<tr class="filas_aleatorias asientosok">
					<td>
						'.$contador.'<input type="text" name="codigo[]" class="added" value="'.$row2['idLocalidad'].'" />
						<input type="hidden" name="row[]" class="added" value="'.$i.'" />
						<input type="hidden" name="col[]" class="added" value="'.$y.'" />
					</td>
					<td>
						<input type="text" name="chair[]" class="added" value="'.$asientos.'" />
					</td>
					<td>
						<input type="text" name="des[]" class="added" value="'.$row2['strDescripcionL'].'" />
					</td>
					<td>
						<input type="text" name="num[]" class="added" value="1" />
					</td>
					<td>';
					if($numrows > 0){
						$pre = $row1['doublePrecioPreventa'];
						$tot = $row1['doublePrecioPreventa'];
						$content .= '
						<input type="text" name="pre[]" class="added" value="'.$row1['doublePrecioPreventa'].'" />
						<input type="text" name="tot[]" class="added" value="'.$row1['doublePrecioPreventa'].'" />';	
					}else{
						$pre = $row2['doublePrecioL'];
						$tot = $row2['doublePrecioL'];
						$content .= '
						<input type="text" name="pre[]" class="added" value="'.$row2['doublePrecioL'].'" />
						<input type="text" name="tot[]" class="added" value="'.$row2['doublePrecioL'].'" />';
					}
					$content .= '
					</td>
				</tr>';
