		...
		for($y = 1; $y <= 30; $y++){
			...
			if(in_array($y,$arr[$ff][$y])){
				...
				echo '<td style="vertical-align:middle;">
						<div style="background-color:#fff; text-align:center; font-size:10px; width:30px; height:30px; color:#000; border:1px solid #000; cursor:pointer;" class="inputchk'.$id_local.' asiento_vasio" id="A-'.$id_local.'-'.$ff.'-'.$y.'" onclick="add_asientos('.$id_local.','.$id_concierto.',\''.$ff.'\','.$y.')">
							'.$y.'
						<input type ="hidden" class = "contiene_pos_'.$id_local.'_'.$ff.'_'.$y.'" value = ""/>
						</div>
					</td>';
			}
		}
