		...
		<div style="background-color:#171A1B;"  ><br/><br/>
			<div class = 'row'>
				...
				<div class = 'col-xs-5' style = 'color:#fff;'>
					<div class="content" style = 'text-align:center;'>
						<h2 style = 'padding:10px;background-color:#ED1568;color:#fff;font-size:16px'>ENTRAGA DE BOLETOS A PUNTO DE VENTA</h2>
						<br/>
						<input type = 'text' id = 'num_entradas' class = 'form-control entero' placeholder = 'Ingrese el N° de boletos entregados'/>
						<br/>
			...
			<div class="row">
				...
				<div class="col-md-6">
					...
					<?php
						$sqlDist = 'SELECT idDistribuidor, nombreDis FROM distribuidor';
						$resDist = mysql_query($sqlDist) or die (mysql_error());
						while ($rowDist = mysql_fetch_array($resDist)) {
							$sqlUsu = 'SELECT idUsuario FROM Usuario WHERE strObsCreacion = '.$rowDist['idDistribuidor'].'';
							$resUsu = mysql_query($sqlUsu);
							while ($rowUsu = mysql_fetch_array($resUsu)) {
								echo '<option value="'.$rowUsu['idUsuario'].'"">'.$rowDist['nombreDis'].'</option>';
							}
						}
					?>
					</select>
