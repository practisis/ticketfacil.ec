...
<?php
	...
	for($i=0;$i<=count($compras)-1;$i++){	
		...
		$query1 = 'INSERT INTO Boleto VALUES ("NULL","'.$seccion.'","'.$code.'",'.$idcli.','.$concert_id.','.$compras[$i]['codigo'].',"'.$valorPago.'","'.$quien_vendio_boleto.'", "0", "'.$espreventa.'", "2" , "","'.$strDocumentoC.'","A","S","'.$numeroSerie.'", "'.$numeroSerie_localidad.'", "'.$identComprador.'" , "'.$_SESSION['iduser'].'")';
		//echo $query1."<br/><br/>";
		...
		$trans = 'INSERT INTO transaccion_distribuidor VALUES ("NULL", '.$idcli.', '.$concert_id.', "'.$compras[$i]['codigo'].'", "'.$_SESSION['idDis'].'", "1","'.$idboleto.'", "tarjeta", "'.$compras[$i]['pre'].'","","")';
		//echo $trans;
		// exit;
							...
							<table align="center" style="width:100%; border-collapse:separate; border-spacing:15px 5px;font-size:11px;text-transform:uppercase;">
								...
								<tr>
									<td style="text-align:center;">
										...
										Esto es un comprobante de pago en l&iacute;nea , mediante su cuenta de PAYPAL:
									</td>
								</tr>
