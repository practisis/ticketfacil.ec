<?php
	...
	//echo '<input type="hidden" id="concert_id" value="'.$idConcierto.'" />';
...
<script type="text/javascript">
	Stripe.setPublishableKey('pk_live_8IuPDxUM3GTxiUbeywIIObrm');
	...
	var stripeResponseHandler = function(status, response) {
		...
		if(response.error){
			...
			$.post('subpages/Compras/ajaxChargeCard_Kiosko.php',{
				token : token , concert_id : concert_id , total : total , cant : cant , valFor : valFor , idcli : idcli
			}).done(function(data){
				console.log(data)
				$('#alerta1').fadeIn();
				$('#aviso').modal('show');
		...
		<div style="border: 2px solid #000; margin:20px;">
			<div style="margin:40px; border:2px solid #000; color:#000; font-size:20px;">
				...
				<div class="row">
					...
					<div class="col-lg-6 titleright" style="margin:0px; padding:10px;">
						<p><?php echo number_format(($preciototal),2);?></p>
					</div>
				</div>
			</div>
				...
				<form action="" method="POST" id="payment-form">
					...
					<div class="row">
						...
						<div class="col-lg-6" style="margin:0px; text-align:center; padding:10px;">
							...
							<input class="form-control" value = '4242424242424242' placeholder="Digita el número de tu tarjeta" onkeydown="justInt(event,this);" type="text" maxlength="20" data-stripe="number"/>
						</div>
					</div>
					<div class="row">
						<div class="col-lg-6" style="margin:0px; text-align:center; padding:10px;">
							<p><strong>C&oacute;digo de Seguridad:</strong></p>
							<input value = '123' class="form-control" placeholder="CVC" type="password" onkeydown="justInt(event,this);" maxlength="4" data-stripe="cvc"/>
						</div>
						<div class="col-lg-6" style="margin:0px; text-align:center; padding:10px;">
							<p><strong>Fecha expiraci&oacute;n (MM/AAAA):</strong></p>
							<div class="row" style="padding-left: 80px;">
								<div class="col-xs-4" style="margin:0px; padding:0px 5px;">
									<input value = '11' class="form-control" placeholder="Mes(MM)" type="password" maxlength="2" onkeydown="justInt(event,this);" data-stripe="exp-month"/>
								</div>
								<div class="col-xs-1" style="margin:0px; padding:0px 5px;">
									<span><strong>/</strong></span>
								</div>
								<div class="col-xs-5" style="margin:0px; padding:0px 5px;">
									<input value = '2020' class="form-control" placeholder="Año(AAAA)" type="password" maxlength="4" onkeydown="justInt(event,this);" data-stripe="exp-year"/>
								</div>
							</div>
						</div>
					</div>
...
function aceptarModal(){
	...
	if(!$('#alerta1').is(':hidden')){
		window.location = '?modulo=pagoTarjetaCreditoOk';
