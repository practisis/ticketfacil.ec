<?php
	...
	$estado = "No";
	$queryCli = "	SELECT strNombresC, strDocumentoC, strMailC, strDireccionC, strCiudadC, intTelefonoMovC , idDeposito
					FROM Deposito 
					JOIN Cliente 
					ON Deposito.idCli = Cliente.idCliente 
					WHERE idFactura = '$intDeposito' 
					AND strDepositoD = '$estado'
				";
	// echo $queryCli."<br>";
	$resCli = $mysqli->query($queryCli);
	$queryDep = "	SELECT idDeposito, intNumBoleto, intFilaD, intColD, strDescripcionL, doublePrecioL , strCaracteristicaL , idLocalidad , valor
					FROM Deposito 
					JOIN Localidad 
					ON Deposito.idLoc = Localidad.idLocalidad 
					WHERE idFactura = '$intDeposito' 
					AND strDepositoD = '$estado'";
	// echo $queryDep;
	$resDep = $mysqli->query($queryDep);
	$queryCon = "	SELECT strEvento, dateFecha, timeHora 
					FROM Deposito 
					JOIN Concierto 
					ON Deposito.idCon = Concierto.idConcierto
					WHERE idFactura = '$intDeposito' 
					AND strDepositoD = '$estado'";
	...
	// echo $registros_cli."aqui <br>";
	if($registros_cli == 0){
		echo "<script>alert('Tu deposito ya fue registrado gacias');window.location='?modulo=start';</script>";
	}
	...
	$selectPrePreventa = "	SELECT doublePrecioPreventa 
							FROM Deposito 
							JOIN Localidad 
							ON Deposito.idLoc = Localidad.idLocalidad 
							JOIN Concierto 
							ON Deposito.idCon = Concierto.idConcierto 
							WHERE dateFechaPreventa >= CURRENT_DATE 
							AND idFactura = '$intDeposito' 
							AND strDepositoD = '$estado'
						";
	...
	echo '<input type="hidden" id="intDeposito" value="'.$intDeposito.'" />';
	echo '<input type="hidden" id="idDeposito" value="'.$rowCli['idDeposito'].'" />';
	
	
	$sqlF = 'select * from factura where id = "'.$_GET['cod'].'" ';
	$resF = mysql_query($sqlF) or die (mysql_error());
	$rowF = mysql_fetch_array($resF);
	
	$sqlC = 'select * from Concierto where idConcierto = "'.$rowF['idConc'].'" ';
			...
			<div style="background-color:#00ADEF; margin-left:40px; margin-top:20px; margin-right:-42px; position:relative; padding: 20px 50px 10px 20px;">
				<table class="table-response" style="width:100%; color:#fff; font-size:20px; border-collapse: separate; border-spacing: 20px 15px;">
					...
					<?php 
						...
						$sqlSd = 'SELECT count(id) as cuantos FROM `socio_membresia` where cedula = "'.$documento.'" ';
						...
						while($rowDep = mysqli_fetch_array($resDep)){
							...
							if($aplica_descuento_socio == 0){
								...
								$sqlD = 'SELECT * FROM `descuentos` WHERE `idloc` = "'.$rowDep['idLocalidad'].'" AND `nom` LIKE "%socio%" ORDER BY `idloc` ASC ';
								// echo $sqlD."<br>";
								$resD = mysql_query($sqlD) or die (mysql_error());
								$rowD = mysql_fetch_array($resD);
								
								$totalpagar2 += number_format(($rowD['val']), 2,'.','');
								// echo $totalpagar2;
							}
					...
					<tr class="pago_dep">
						...
						<td style="text-align:center;">
							<?php
								if($rowDep['strCaracteristicaL'] == 'Asientos numerados'){
							?>
									Fila-<?php echo $rowDep['intFilaD'];?>_Silla-<?php echo $rowDep['intColD'];?>
							<?php
								}else{
									echo 'Asientos no numerados';
								}
							?>
						...
						<?php 
							...
							// }else{
								// $total = number_format($rowD['val'],2,'.','');
							// }
						?>
						<td style="text-align:center">
							<?php echo $rowDep['valor'];?>
						</td>
					</tr>
					<?php
						$suma += $rowDep['valor'];
					...
					?>	
						...
						<tr>
							...
							<td style="text-align:center">
								<strong><?php echo (number_format(($rowC['envio']),2));?></strong>
							</td>
						</tr>
						
						<tr>
							<td colspan="4" style="text-align:right">
								<strong>Comisión de Transacción : </strong>
							</td>
							<td style="text-align:center">
								<strong>
									<?php 
										$comis_transfer = (((($precio_total + $rowC['envio']) * $rowC['porce_transfer'])/100) + $rowC['comis_transfer']);
										echo number_format(($comis_transfer),2);
									?>
								</strong>
							</td>
						</tr>
						...
						<tr>
							...
							<td style="text-align:center">
								<strong><?php echo number_format(($precio_total + $rowC['envio'] + $comis_transfer),2);?></strong>
							</td>
						</tr>
					...
					?>
						...
						<tr>
							...
							<td style="text-align:center">
								<strong>
									<?php 
										$comis_transfer = ((($precio_total * $rowC['porce_transfer'])/100) + $rowC['comis_transfer']);
										echo ($comis_transfer);
									?>
								</strong>
							</td>
						...
						<tr>
							...
							<td style="text-align:center">
								<strong><?php echo ($precio_total + $comis_transfer);?></strong>
							</td>
						</tr>
					<?php
						}
...
<script type="text/javascript">
	...
	function enviar(){
		...
		if (!ndep || !fecha) {
			alert('Debe ingresar un numero de deposito y fecha validos');
		}else{
			var intDeposito = $('#intDeposito').val();
			$.post('subpages/Compras/inserts/add_deposito.php',{
				ndep : ndep, fecha : fecha, intDeposito : intDeposito , idDeposito : idDeposito
			}).done(function(data){
				if (data == 1) {
					alert('Ya se uso este numero de deposito');
				}else if(data == 2){
