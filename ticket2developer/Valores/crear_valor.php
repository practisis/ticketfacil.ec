<?php
// SABER QUIEN ES EL SOCIO QUE INGRESO  
				...
				<div style="background-color:#00ADEF; margin:20px -42px 0px 40px; position:relative; padding:10px 40px 20px; color:#fff;">
					...
					<div class="row">
						<div class="col-lg-10">
							...
							<?php 
							// SELECCIONAR TODOS LOS EQUIPOS QUE COINCIDAN CON LA ID DEL SOCIO
							include '../conexion.php';
							$sel = mysql_query("SELECT *FROM equipo WHERE mail = '$nombre'");
							?>
							<select class="form-control inputlogin" id="perfil" name="perfil" required="required">

								<option value="0">Seleccione...</option>
								<?php while ($row = mysql_fetch_array($sel)){?>
								<option value="Admin" onClick="muestra_oculta('contenido')"><?php echo $row['nombre'];?></option>
								<?php } ?>
							</select>
						</div>
					...
					<div style="text-align:center; margin:20px; padding:20px 0;">
						<button type="button" class="btn btn-primary" onClick="muestra_oculta('contenido')" title="">Siguiente</button>
					</div>
				</div>
				<!--FIN DE CREAR EQUIPO-->
