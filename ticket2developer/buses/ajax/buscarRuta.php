<?php
	...
	$_SESSION['ciudad_salida'] = $salida;
	$_SESSION['ciudad_llegada'] = $llegada;
	...
	if($_REQUEST['id_coop'] != ''){
		...
		$_SESSION['id_coop_vuelta'] = $id_coop;
	...
	if($fecha_regreso != ''){
		$_SESSION['saber_ida_vuelta'] = 1;
		$_SESSION['salida_vuelta'] = $_REQUEST['llegada'];
		$_SESSION['llegada_vuelta'] = $_REQUEST['salida'];
		$_SESSION['terminal_vuelta'] = $_REQUEST['terminal'];
		$_SESSION['terminal_vuelta_salida'] = $_REQUEST['terminal_llega'];
		$_SESSION['fecha_vuelta'] = $_REQUEST['fecha_regreso'];
		
		echo "<input type = 'hidden' id = 'salida_vuelta' value = 'salida de regreso : ".$_SESSION['salida_vuelta']."' />";
		echo "<input type = 'hidden' id = 'llegada_vuelta' value = 'llegada de regreso : ".$_SESSION['llegada_vuelta']."' />";
		echo "<input type = 'hidden' id = 'terminal_vuelta' value = 'terminal salida al regreso : ".$_SESSION['terminal_vuelta']."' />";
		echo "<input type = 'hidden' id = 'terminal_vuelta_salida' value = 'terminal llegada al regreso".$_SESSION['terminal_vuelta_salida']."' />";
		echo "<input type = 'hidden' id = 'fecha_vuelta' value = 'fecha salida de regreso : ".$_SESSION['fecha_vuelta']."' />";
		echo "<input type = 'hidden' id = 'fecha_vuelta' value = 'Cooperativa : ".$_SESSION['id_coop_vuelta']."' />";
		echo "<input type = 'hidden' id = 'saber_ida_vuelta' value = 'hay regreso : ".$_SESSION['saber_ida_vuelta']."' />";
	}else{
		$_SESSION['salida_vuelta'] = '';
		$_SESSION['llegada_vuelta'] = '';
		$_SESSION['terminal_vuelta'] = '';
		$_SESSION['fecha_vuelta'] = '';
		$_SESSION['terminal_vuelta_salida'] = '';
		$_SESSION['saber_ida_vuelta'] = 0;
		echo "<input type = 'hidden' id = 'salida_vuelta' value = 'salida de regreso : ".$_SESSION['salida_vuelta']."' />";
		echo "<input type = 'hidden' id = 'llegada_vuelta' value = 'llegada de regreso : ".$_SESSION['llegada_vuelta']."' />";
		echo "<input type = 'hidden' id = 'terminal_vuelta' value = 'terminal salida al regreso : ".$_SESSION['terminal_vuelta']."' />";
		echo "<input type = 'hidden' id = 'terminal_vuelta_salida' value = 'terminal llegada al regreso : ".$_SESSION['terminal_vuelta_salida']."' />";
...
<?php
	...
	?>
					<tr style='text-transform:capitalize;'>
						<td style = 'padding-left: 10px;padding-right: 10px' width = '75%' >
							<table width = '100%' height = '100%'>
								<tr>
									<td>
										<img src = 'images/busIdaV.png'/>
									</td>
									<td>
										...
										<table width = '100%' height = '100%'>
											<tr>
												<td valign = 'top' style = 'padding-top: 5px'>
													<div style='color:#fff;font-size:12px;background-color:#0386A4;padding-left: 5px;position: relative; left: -1px;'>
														hora de salida<br>
														<i class="fa fa-clock-o" aria-hidden="true" style='color:rgb(172,170,171);font-size:12px;'></i>   <?php echo $row['hora'];?>
													</div>
												</td>
												<td valign = 'top'>
										...
										<table>
											<tr>
													...
													<?php
														...
														if($numero_filas == 0){
															...
															while($rowEs = mysql_fetch_array($resEs)){
																$sqlCiuEsc = 'select * from ciudades where id = "'.$rowEs['id_ciu_sal'].'" ';
																//echo $sqlCiuEsc."<br/>";
																$resCiuEsc = mysql_query($sqlCiuEsc) or die (mysql_error());
																$rowCiuEsc = mysql_fetch_array($resCiuEsc);
																
																
																$sqlCiuEsc1 = 'select * from ciudades where id = "'.$rowEs['id_ciu_lleg'].'" ';
																//echo $sqlCiuEsc."<br/>";
																$resCiuEsc1 = mysql_query($sqlCiuEsc1) or die (mysql_error());
																$rowCiuEsc1 = mysql_fetch_array($resCiuEsc1);
																
																
																
																$nomCiuEsc .= "- <span style='font-size:8px;color:rgb(172,170,171);'>".$rowCiuEsc['nom']."-".$rowCiuEsc1['nom']." -</span><br/>";
															}
												...
												<td valign = 'top' style ='padding-left:10px;'>
													...
													<i class="fa fa-clock-o" aria-hidden="true"></i>   <span style='color:rgb(172,170,171);font-size:12px;'><?php echo $row['hora_llega'];?></span>
												</td>
											</tr>
										</table>
									</td>
	...
	//	echo $sqlEsc."<br/>";
		...
		if(mysql_num_rows($resEsc)>0){
			...
			//	echo $sql2;
				...
				$sqlB = 'select asientos from bus where codigo = "'.$row['id_bus'].'" ';
				$resB = mysql_query($sqlB) or die (mysql_error());
				$rowB = mysql_fetch_array($resB);
				$numAsientosCreados = $rowB['asientos'];
				if($numAsientosOcupados >= $numAsientosCreados ){
										...
										<table width = '100%' height = '100%'>
											<tr>
												<td valign = 'top' style = 'padding-top: 5px'>
													<div style='color:#fff;font-size:12px;background-color:#0386A4;padding-left: 5px;position: relative; left: -1px;'>
														hora de salida 
														<i class="fa fa-clock-o" aria-hidden="true"></i>   <?php echo $row['escala_hora_salida'];?>
													</div>
												</td>
												<td valign = 'top' style = 'padding-top: 5px'>
													<span style = 'color:#0386A4;'><?php echo $row2['nom'];?></span><br/>
										...
										<table>
											<tr>
												...
												<td valign = 'top' style ='padding-left:10px;'>
													<?php echo $row3['nom'];?><br/>
													<span style='color:rgb(172,170,171);font-size:12px;'><?php echo $row5['nombre'];?></span><br/>
													<i class="fa fa-clock-o" aria-hidden="true" style='color:rgb(172,170,171);font-size:12px;'></i>   <span style='color:rgb(172,170,171);font-size:12px;'><?php echo $row['hora_llega'];?></span>
												</td>
											</tr>
										</table>
									</td>
		...
		<?php
			...
			echo '<h1 style = "color:#fff;text-transform:capitalize;" >no hay disponiilidad   !!!</h1>';
		}
	}
	?>
	</table>
	<script>
		function enviaBus(id , id_bus , fecha , fecha_regreso , id_coop){
			...
			window.location = "?page=bus_paso_1&id="+id+"&id_bus="+id_bus+"&fecha="+fecha+"&fecha_regreso="+fecha_regreso+"&regr="+regr+"&id_coop="+id_coop+"&n="+normales+"&e="+especiales;
		}
	</script>