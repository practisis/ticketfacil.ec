			...
			if(($.trim(response) == 'noexiste') || ($.trim(response) == 'nodata')){
				...
				$('#personas_boletos').append('<div class="row" style="color:#fff;">\
						...
						<div class="col-lg-5">\
							...
							<input type="text" class="inputlogin form-control" id="documento" name="documento" placeholder="Cédula" value="'+newcedula+'"  onchange="ValidarDocumento(this,this.value)" />\
						</div>\
					</div>');
				for(var j = 1; j < num_personas; j++){
					...
					$('#personas_boletos').append('<div class="row datos_cliente_boleto" style="color:#fff;">\
						<div class="col-lg-5">\
							...
							<input type="text" class="inputlogin form-control nombres nom'+j+'" onkeyup="ponernombre('+j+',this.value)" placeholder="Nombre del Cliente" onkeypress="validarNombres(this,'+j+')" />\
						</div>\
						<div class="col-lg-5">\
							<h4 style="color:#fff;">&nbsp;</h4>\
							<h4>Documento (C.I./R.U.C/Pasaporte):</h4>\
							<input type="text" class="inputlogin form-control documento" placeholder="Cédula" value="'+newcedula+'" onchange="ValidarDocumento(this,this.value)" onkeyup="validarRepetidos(this,'+j+',this.value)" />\
				...
				for(var i = 0; i <= (obj.Cliente.length -1); i++){
					...
					for(var j = 1; j < num_personas; j++){
						...
						var des = $('#des'+j).val();
						$('#personas_boletos').append('<div class="row datos_cliente_boleto" style="color:#fff;">\
							<div class="col-lg-5 datos">\
								<h4 style="color:#fff;"><strong>TICKET: '+asiento+' ('+des+')</strong></h4>\
								<h4>Nombre y Apellido:</h4>\
								<input type="text" class="inputlogin form-control nombres nom'+j+'" onkeyup="ponernombre('+j+',this.value)" placeholder="Nombre del Cliente" value="'+nombre+'" onkeypress="validarNombres(this,'+j+')" />\
							</div>\
							<div class="col-lg-5 datos">\
								<h4 style="color:#fff;">&nbsp;</h4>\
								<h4>Documento (C.I./R.U.C/Pasaporte):</h4>\
								<input type="text" class="inputlogin form-control documento cambio'+j+'" placeholder="Cédula" value="'+cedula+'" onchange="ValidarDocumento(this,this.value)" onkeyup="validarRepetidos(this,'+j+',this.value)" />\
...
function validarRepetidos(t,id,value){
	var otracedula = value;
	var otronombre = $('.nom'+id).val();
	// alert(otracedula+' - '+otronombre);
	$('.datos_cliente_boleto').each(function(){
		var nombre = $(this).find('.nombres').val();
		var cedula = $(this).find('.documento').val();
		if((nombre != otronombre) && (otracedula == cedula)){
			// alert(nombre+' - '+cedula+' // '+otronombre+' - '+otracedula);
			$(t).val('');
			$('#alerta').modal('show');
		}
	});
	$('#documento'+id).val(value);
}
	...
	if((tipopago == '') || (valores_cliente == '') || (mail == '') || (dir == '') || (forma == 0) || (nombres == '') || (documento == '')){
		...
		if((clienteok == 'ok') && (idcliente != '')){
			...
			$.post('distribuidor/ventas/ajax/ajaxPagoDistribuidor.php',{
				clienteok : clienteok, idcliente : idcliente, concierto : concierto, valores : valores_form, tipopago : tipopago, forma : forma, mail : mail, movil : movil, dir : dir,valores_cliente : valores_cliente_form, num_boletos : num_boletos
			}).done(function(response){
			...
			$('#procesando').delay(600).fadeIn();
			$.post('distribuidor/ventas/ajax/ajaxPagoDistribuidor.php',{
				clienteok : clienteok, mail : mail, movil : movil, dir : dir, concierto : concierto, valores : valores_form, tipopago : tipopago, forma : forma, valores_cliente : valores_cliente_form, documento : documento, nombres : nombres, num_boletos : num_boletos
			}).done(function(response){
				if($.trim(response) == 'error'){
					$('#alerta6').fadeIn();
					$('#aviso').modal('show');
