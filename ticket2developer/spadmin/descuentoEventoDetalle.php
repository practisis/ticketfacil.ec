	...
	<div style="background-color:#171A1B; padding:20px;">
		<div style="border: 2px solid #00AEEF; margin:20px;">
			...
			<div style="background-color:#00ADEF; margin:20px -42px 10px 40px; position:relative; padding:10px; text-align:center; color:#fff; font-size:18px;">
				<strong>
					<a href="?modulo=descuentoEvento&id=<?php echo $_REQUEST['id'];?>" target="_top">
						<button type="button" class="btn btn-danger">&nbsp;Nuevo Descuento&nbsp;</button>
					</a>
				</strong>

                <strong>
						<button type="button" class="btn btn-danger" onclick="verborrardescuentos();">&nbsp;Borrar Descuentos&nbsp;</button>
				</strong>
            ...
            <div class = 'col-md-12'>
    			<table class='table'>
    					<tr>
                            ...
                            <td width = '200px'>
                                <br>
                                <button type="button" class="btn btn-danger" onclick="borradescuento(<?php echo $_REQUEST['id'];?>);">&nbsp;Borrar Todo&nbsp;</button>
    						</td>
                        </tr>
                        <tr>
    						<td></td>
		...
		<div class = 'row'>
			...
			<div class = 'col-md-12'>
				...
				<?php
					$sql = 'select l.strDescripcionL , d.* , l.doublePrecioL , l.idLocalidad , d.id as id_desc
					...
					while($row = mysql_fetch_array($res)){
						...
						$sqlV = 'SELECT * FROM valor_localidad_ventas where id_loc = "'.$row['idLocalidad'].'" and id_desc = "'.$row['id_desc'].'" ';
						...
						$numVentas = mysql_num_rows($resV);
						// echo $numVentas." <<>> ";
						if($numVentas == 0){
							$valorVenta = $row['val'];
						}else{
							$valorVenta = $rowV['valor'];
						}
				...
				?>
							<tr class = 'descuentos_global' numero_descuento = '<?php echo $param;?>' >
								...
								<td>
									<input onkeyup = "saberEspecial(<?php echo $param;?>)" type = 'text' value = '<?php echo $row['nom'];?>' class = 'form-control' id = 'nombDesc_<?php echo $param;?>' style = 'color:#000;' placeholder = 'Nombre Desc.'/>
								</td>
								<td>
									<?php echo $row['doublePrecioL'];?>
								</td>
								<td>
									<input type = 'text' value = '<?php echo $row['val'];?>' class = 'form-control entero' id = 'valorDesc_<?php echo $param;?>' style = 'color:#000;' placeholder = 'Valor Dec.'/>
									<input type = 'hidden' value = '<?php echo $row['idLocalidad'];?>' id = 'idLoc_<?php echo $param;?>' style = 'color:#000;'/>
								</td>
								<td>
									<input type = 'text' value = '<?php echo $valorVenta;?>' class = 'form-control entero' id = 'valorVenta_<?php echo $param;?>' style = 'color:#000;' placeholder = 'Valor Dec.'/>
								</td>
								<td>
									<input type = 'checkbox' <?php echo $che;?> value = '<?php echo $row['est'];?>' id = 'estDesc_<?php echo $param;?>'/>
								</td>
								
								<td>
									<input type = 'checkbox' <?php echo $check;?> value = '<?php echo $row['web'];?>' id = 'estDescWeb_<?php echo $param;?>'/>
								</td>
								
								<td>
									<button type="button" class="btn btn-<?php echo $btn;?>" onclick = 'editaDescuento( 1 ,  <?php echo $row['idLocalidad'];?> , <?php echo $param;?> )' ><?php echo $txt.'  ['.$param.']';?></button>
									<button type="button" class="btn btn-danger" onclick = 'editaDescuento( 2 ,  <?php echo $row['idLocalidad'];?> , <?php echo $param;?> )' >BORRAR</button>
								</td>
							</tr>
				<?php
				...
				?>
					<tr>
						<td colspan = '9' style = 'text-align:center;'>
							<button type="button" id = 'graba_todos' class="btn btn-success" onclick = 'grabarTodos()'>Grabar Todos</button>
						</td>
					</tr>
				</table>
			</div> 
							...
							?>
								<tr class = 'cantidades_cortesias' localidad = "<?php echo $row2['idLocalidad'];?>" >
									...
									<td>
										...
										<input onblur = 'controlaCantidad(<?php echo $row2['idLocalidad'];?>)' id = 'cantidad_<?php echo $row2['idLocalidad'];?>' type = "number" class = 'form-control' value = '<?php echo $row2['cantidad'];?>' />
										<input id = 'id_canti_<?php echo $row2['idLocalidad'];?>' type = "hidden" class = 'form-control' value = '<?php echo $id_cantidades;?>' />
										<input id = 'id_canti_desc_<?php echo $row2['idLocalidad'];?>' type = "hidden" class = 'form-control' value = '<?php echo $row2['id_desc'];?>' />
							...
							?>
								<tr>
									<td colspan = '6' align = 'center'>
										<button type="button" id = 'grabarCantidades' class="btn btn-warning" onclick="grabarCantidades()">GRABAR</button>
									</td>
								</tr>
							</table>
						</div>
...
<script>
	...
	function grabarCantidades(){
		$('#grabarCantidades').attr('disabled',true);
		var servi = '';
		var continua = 1;
		...
		if(continua == 0){
			...
			$.post("spadmin/grabaCantidadCortesias.php",{ 
				...
				alert(data)		
				$('#grabarCantidades').attr('disabled',false);			
			});
		}
	...
	function grabarTodos(){
		var servi = ''
		$('#graba_todos').html('Espere enviando información');
		$( ".descuentos_global" ).each(function( index ) {
			...
			var estDesc = 0;
			var valorVenta = $('#valorVenta_'+id).val();
			var idLoc_ = $('#idLoc_'+id).val();
			...
			if(nombDesc == '' || valorDesc == ''){
				...
				servi += id +'@'+ nombDesc +'@'+ valorDesc +'@'+ ident+'@'+ estDesc +'@'+ estDescWeb +'@'+ valorVenta +'@'+ idLoc_ + '|'
			}
		...
		$.post("spadmin/grabarDescuentoTodos.php",{ 
			serviform : serviform
		}).done(function(data){
			$('#graba_todos').html(data);
			alert('Gracias por esperar,' + data);
			window.location = '';
		});
	}
	...
	function editaDescuento(ident , idLoc , id ){
		...
		alert(nombDesc + ' << >> ' + valorDesc + '<< >> ' + id + ' <<  >> ' + valorVenta);
		if(nombDesc == '' || valorDesc == ''){
			
		}else{
			$.post("spadmin/grabarDescuento.php",{
				id : id , nombDesc : nombDesc , valorDesc : valorDesc , ident : ident , 
				estDesc : estDesc , estDescWeb : estDescWeb , valorVenta : valorVenta , idLoc : idLoc
			}).done(function(data){
				alert(data);
				// location.reload();
			});
