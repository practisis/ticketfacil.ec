<?php 
	...
	$sqlCortesia = 'INSERT INTO `cortesias` (`id`, `id_con`, `id_loc`, `num_bol` , `tipo` , fecha ) VALUES (NULL, "'.$concierto.'", "'.$local.'", "'.$numticket.'" , "'.$ident.'" , "'.$hoy2.'" )';
	// echo $sqlCortesia;
	...
	$content = '<table width="500px" align="center" style="border:1px solid #ccc;">';
		$content.='<tr>';
			$content.='<td colspan="3">';
				$content.='Estimado cliente le entregamos los boletos de Cortesia para el concierto de '.$strEvento.' en la localidad de '.$strDescripcionL.'';
			$content.='</td>';
		$content.='</tr>';
	$m = 0;
		...
		for($y = 1; $y <= $row['col']; $y++){
			if(in_array($y,$arr[$i][$y])){
				...
				if($strCaracteristicaL == 'Asientos numerados'){
					...
					$ocupadas = 'Asientos no Numerados';
				}
			...
			//	echo $sqlB1."<br/>";
				...
				$sql2 = 'INSERT INTO Boleto VALUES ("NULL","1","'.$code.'",'.$idCli.','.$concierto.','.$local.',"0","0","0","0","0","cortesia",1212121212,"A","S","'.$numeroSerie.'","'.$numeroSerie_localidad.'", "'.$identComprador.'" , "0")';
				$res2 = mysql_query($sql2) or die (mysql_error());
				$idboleto = mysql_insert_id();
				
				$sqltrans = 'INSERT INTO transaccion_distribuidor VALUES ("NULL", '.$idCli.', '.$concierto.', "'.$local.'", "3", "1","'.$idboleto.'", "", "'.$precio.'","'.$hoy2.'","")';
