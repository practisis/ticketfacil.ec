<?php
	...
	if($param == 1){
		...
		// $color = '';
		// $div = '<div id = "contenedor_numero_asiento_'.$fila.'_'.$columna.'" parametro = "0" id_ocu = "0"  fila = "'.$fila.'" columna = "'.$columna.'"  id_loc = "'.$id_loc.'"  idcon = "'.$idcon.'" onclick = "saberEstado('.$fila.','.$columna.',0,0,'.$id_loc.','.$idcon.')" class = "'.$clase.' " style ="'.$color.'font-size:12px;padding-top:7px !important;border:1px solid #ccc;vertical-align:middle;cursor:pointer;width:30px;height:35px;text-align: center">'.$columna.'</div>';
		echo $param;
	}else{
		$sql = 	'
