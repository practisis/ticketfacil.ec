...
<?php
	...
	while($row = mysql_fetch_array($res)){
		...
		$sqlBX = 'SELECT count(idBoleto) as vendidos_de_baja FROM `Boleto` WHERE `idCon` = "'.$idcon.'0000" AND `idLocB` = "'.$row['idLocalidad'].'" AND `tercera` = 0 '.$query.' ORDER BY `idLocB` DESC ';
		$resBX = mysql_query($sqlBX) or die (mysql_error());
		$rowBX = mysql_fetch_array($resBX);
...
?>
		<tr>
			...
			<td>
				<?php 
					echo $rowBX['vendidos_de_baja'];
					$suma_total_de_baja += $rowBX['vendidos_de_baja']; 
				?>
			</td>
			<td>
				<?php 
...
<?php
	...
	while($rowD = mysql_fetch_array($resD)){
		...
		$sqlLV = 'SELECT * FROM `valor_localidad_ventas` WHERE `id_desc` = "'.$rowD['id'].'" ';
		$resLV = mysql_query($sqlLV) or die (mysql_error());
		$rowLV = mysql_fetch_array($resLV);
...
?>
		<tr>
			...
			<td>
				<?php 
					echo number_format(($rowLV['valor']),2 , "," , "");
					
				?>
			</td>
			<td>
				<?php 
					echo number_format(($rowLV['valor'] * $rowB1['vendidos_descuento']),2 , "," , "");
					$suma_precio_descuento += ($rowLV['valor'] * $rowB1['vendidos_descuento']);
				?>
			</td>
			<td>
				<?php 
					echo number_format((($rowLV['valor'] * $rowB1['vendidos_descuento'])/(1+$rowI['municipio'])),2 , "," , "");
					$suma_valor_admision_descuento += (($rowLV['valor'] * $rowB1['vendidos_descuento'])/(1+$rowI['municipio']));
				?>
			</td>
			
			<td>
				<?php 
					echo number_format(($rowLV['valor'] * $rowB1['vendidos_descuento']) - ((($rowLV['valor'] * $rowB1['vendidos_descuento'])/(1+$rowI['municipio']))),2 , "," , "");
					$suma_total_admision_descuento += (($rowLV['valor'] * $rowB1['vendidos_descuento']) - ((($rowLV['valor'] * $rowB1['vendidos_descuento'])/(1+$rowI['municipio']))));
				?>
			</td>
		</tr>
