		...
		<tr>
			...
			<?php
				...
				// // while($rowLo = mysql_fetch_array($resLo)){
					...
					while($rowD1 = mysql_fetch_array($resD1)){
						...
						$sqlBT1 = '	SELECT count(idBoleto) as vendidos_descuento_todos , Boleto.valor as precio_vendido
									FROM `Boleto`
									WHERE `idLocB` in ('.$rowD1['idloc'].'0000)
									AND `tercera` = 1 
									AND `id_desc` in('.$rowD1['id'].')
									'.$query.'
									ORDER BY `idLocB` DESC ';
						
						$resBT1 = mysql_query($sqlBT1) or die (mysql_error());
						$rowBT1 = mysql_fetch_array($resBT1);
			...
			?>
						<tr>
							...
							<td>
								<?php echo $rowBT1['vendidos_descuento_todos']; ?>
							</td>
							<td>
								<?php
