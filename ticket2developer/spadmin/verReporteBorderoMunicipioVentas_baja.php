		...
		<tr>
			...
			<?php
				...
				while($row = mysql_fetch_array($res)){
					...
					while($rowL1 = mysql_fetch_array($resL1)){
						...
						$sqlB = 'SELECT count(idBoleto) as vendidos_normal FROM `Boleto` WHERE `idCon` = "'.$idcon.'0000" AND `idLocB` = "'.$rowL1['idLocalidad'].'" and tercera = 0 and id_desc != "'.$rowDD['id'].'" '.$query.' ORDER BY `idLocB` DESC ';
						// echo $sqlB."<br><br>";
			...
			?>
					<tr class = 'valores_macros' >
						...
						<td>
							<?php
								echo "<div style='background-color: #000;text-align: center; color:green;font-weight: bold;font-size: 18px;' class = 'vendidos_macro' >".$sumavendidos_normal."</div>";
