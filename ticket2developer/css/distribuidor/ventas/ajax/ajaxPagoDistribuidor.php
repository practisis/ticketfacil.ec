	...
	if($_REQUEST['clienteok'] == 'ok'){
				...
				if(mysql_num_rows($resA)>0){
					...
					if($rowB['serieB'] == null || $rowB['serieB'] == '' ){
						...
						if($numeroSerie <= $rowA['secuencialfinalA']){
							...
							$sqlB2 = 'select max(CAST(serie AS INTEGER)) as serieB from Boleto where idCon = "'.$idconcierto.'"  order by idBoleto DESC limit 1';
							echo $sqlB2;
							$resB2 = mysql_query($sqlB2) or die (mysql_error());
							$rowB2 = mysql_fetch_array($resB2);
							if($rowB2['serieB'] == null || $rowB2['serieB'] == '' ){
								$numeroSerie1 = 1;
							}else{
								$numeroSerie1 = ($rowB2['serieB'] + 1);
							}
							...
							$insert2 = 'INSERT INTO Boleto VALUES ("NULL","'.$rand.'","'.$code.'",'.$idcliente.','.$idconcierto.','.$exp[0].',"'.$valorPago.'","'.$quien_vendio_boleto.'","'.$tercera.'",'.$exp[7].',"venta","S","'.$numeroSerie1.'","'.$sitiene.'")';
							//echo $insert2."<br>";
							// exit;
							...
							if($hoy <= $dateFechaPreventa){
								echo 'si esta vigente la preventa<br>';
								$localidad = 'SELECT * FROM Localidad WHERE idLocalidad = "'.$exp[0].'"';
								$lokRow= $gbd -> prepare($localidad);
								$lokRow -> execute();
								$rowLoc = $lokRow -> fetch(PDO::FETCH_ASSOC);
								$precioVenta = $rowLoc['doublePrecioPreventa'];

								$insertCompras_preventa = 'INSERT INTO compras_preventa VALUES (NULL, "'.$exp[0].'", "'.$rand.'" , "'.$idboleto.'", "'.$hoy.'" , "'.$precioVenta.'" )';
								echo $insertCompras_preventa;
								$resCompras_preventa = mysql_query($insertCompras_preventa) or die (mysql_error());
								
								
								
							}else{
								echo 'no esta vigente la preventa';
							}
							
							$boletos .= $idboleto."|";
							if($envio == 'Domicilio'){
							$insert3 = 'INSERT INTO domicilio VALUES ("NULL", "'.$exp[4].'", "'.$exp[5].'", "1", "'.$exp[0].'", '.$idconcierto.', '.$idcliente.', '.$idboleto.', "'.$formapago.'", "'.$dir.'", "'.$exp[6].'", "'.$exp[7].'", "A")';
							// echo $insert3;
							// exit;
						...
						//if($envio != 3){
							...
							echo'lo sentimos ya no disponemos de mas boletos ';
						}
				}else{
					...
					// <tr>
						// <td>
							...
							// <table width="100%" border="1" align="center">
								...
								// </tr>';
								// foreach(explode('@', $_REQUEST['valores']) as $value1){
								// $exp1 = explode('|',$value1);
								// $content3 .= '
								// <tr>
									// <td>'.$exp1[2].'</td>
		...
		foreach(explode('@', $_REQUEST['valores']) as $value){
			...
			try{
				...
				$insert2 = 'INSERT INTO Boleto VALUES ("NULL","'.$rand.'","'.$code.'",'.$idcliente.','.$idconcierto.','.$exp[0].',"'.$valorPago.'","'.$quien_vendio_boleto.'","'.$tercera.'",'.$exp[7].',"venta","S","'.$numeroSerie.'","'.$sitiene.'")';
				$ins2 = $gbd -> prepare($insert2);
				$ins2 -> execute();
				$idboleto = $gbd -> lastInsertId();
				...
				// $stmttrans = $gbd -> prepare($trans);
				// $stmttrans -> execute(array('NULL',$idcliente,$idconcierto,$exp[0],$_SESSION['idDis'],1,$formapago,$exp[3]));
