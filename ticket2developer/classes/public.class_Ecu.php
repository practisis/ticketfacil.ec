...
class InitTicket{
	...
	public $subpagePath;
	public $pages = array(
		'start' => array('path' => 'subpages/startEcu.php','title' => ''),
		...
		'eventos' => array ('path' => 'subpages/conciertos_ecu.php', 'title' => ''),
		...
		'vederDis_2' => array('path' => 'distribuidor/ventas/venderdistribuidor2.php', 'title' => ''),
	...
	$selectDatosConciertoStart = "SELECT idConcierto, strImagen, dateFecha, strEvento, strLugar, timeHora, strDescripcion , tipo_conc , tiene_permisos , es_publi FROM Concierto WHERE  strEstado = ? AND costoenvioC = 1 and idUser = 43 ORDER BY 3 DESC";  //dateFecha >= ? AND
	$resultDatosConciertoStart = $gbd -> prepare($selectDatosConciertoStart);
	$resultDatosConciertoStart -> execute(array($estado));
	
	
	$selectDatosConciertoStart1 = "SELECT idConcierto, strImagen, dateFecha, strEvento, strLugar, timeHora, strDescripcion , tipo_conc , tiene_permisos , es_publi FROM Concierto WHERE  dateFecha >= ? AND strEstado = ? AND costoenvioC = 1 and idUser = 43 ORDER BY dateFecha DESC";  //
	$resultDatosConciertoStart1 = $gbd -> prepare($selectDatosConciertoStart1);
	$resultDatosConciertoStart1 -> execute(array($hoy,$estado));
	
	$selectSliderStart = "SELECT idConcierto, strImagen , es_publi FROM Concierto WHERE costoenvioC = 1 and idUser = 43 ORDER BY dateFecha DESC";
	$resultSlideStar = $gbd -> prepare($selectSliderStart);
	$resultSlideStar -> execute();
	$num_imagenes = $resultSlideStar -> rowCount();
	
	$slidertriple = "SELECT idConcierto, strImagen , es_publi FROM Concierto WHERE costoenvioC = 1 and idUser = 43 ORDER BY dateFecha DESC";
