<?php 
	...
	$sqlTC = 'select * from Concierto where idConcierto = "'.$_REQUEST['idCon'].'" ';
	...
	$sqlB = 'select * from Boleto where idCon = "'.$rowTC['idConcierto'].'"  order by idBoleto DESC limit 1';
	...
	if($hoy <= $dateFechaPreventa){
		$localidad = 'SELECT * FROM Localidad WHERE idLocalidad = "'.$rowB['idLocB'].'"';
	...
	if(image_exists('http://ticketfacil.ec/ticket2/pagekiosko/subpages/Compras/barcode/'.$rowB['strBarcode'].'.png') == 1){
		...
		$hoy = date("Y-m-d");
		$sqlImp = 'INSERT INTO `imp_boleto` (`id`, `idbo`, `idCli`, `fecha` ,codbarras) VALUES (NULL, "'.$rowB['idBoleto'].'", "'.$rowB['idCli'].'", "'.$hoy.'" , "'.$rowB['strBarcode'].'")';
		...
		<page>
			<div style="padding-bottom:30px;border:1px solid #ccc;border-radius:10px;background-color:#282B2D;width:100%;heigth:100%;background-image:url(http://ticketfacil.ec/ticket2/spadmin/'.$rowTC['strImagen'].');background-position: 50 50;">
				...
				<div style="padding-left:10px;padding-rigth:10px;background-color:#fff;color:#000;width:position:relative;margin-left:150px;">
					...
					<p align="center">En la localidad: <strong>'.$row2['strDescripcionL'].'</strong> costo: <strong>'.$precioVenta.'</strong></p>
				...
				<table align="center" style="width:100%; border-collapse:separate; border-spacing:15px 5px;">
					...
					<tr>
						<td style="text-align:center;">
							<br>
							<br>
							<span style="font-size:20px;"><strong>GRACIAS, SE HA COMPLETADO EL PAGO DE SU TICKET ($'.$precioVenta.') CON EXITO</strong></span>
						</td>
					</tr>
		...
		if(!$mail->Send()){
			...
			echo 'segun la configuracion de este concierto , se ha enviado un email al cliente con su boleto!!!';
		}
