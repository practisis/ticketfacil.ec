<?php 
	...
	echo '<input type="hidden" id="data" value="97" />';
	
	
	echo $_SESSION['perfil']." hola <br>";
	echo $_SESSION['idDis']."hola";
?>
<style>
		...
		<div style="background-color:#171A1B; padding:20px;">
			<div style="border: 2px solid #00AEEF; margin:20px;">
				<div style="background-color:#00ADEF; color:#fff; margin:20px 400px 0px 0px; padding-left:30px; font-size:22px;">
					<p><strong>MODULO PARA REIMPRIMIR BOLETOS EN LOTE</strong></p>
				</div>
				<div class="row ">
						...
						<div class = 'col-md-9'>
							<div class="panel panel-info panel-dark">
								<div class="panel-heading">
									<span class="panel-title"><span class="label label-tag label-warning">reimpresion de tickets en lote</span></span>
									<div class="panel-heading-controls">
										<div class="panel-heading-icon"><i class="fa fa-inbox"></i></div>
									</div>
									<button type="button" id="" class="btn btn-warning" onclick = '$("#dar_de_baja_lote").modal("show")'>REIMPRESIONES</button>
								</div>
								...
								<div data-keyboard="false" data-backdrop="static" class="modal fade" id="dar_de_baja_lote" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" style = 'display:none;padding-right:2%;padding-left:2%' >
									<div class="modal-dialog" role="document" style = 'width:100%;' >
										<div class="modal-content">
											<div class="modal-header">
												<button type="button" class="close" data-dismiss="modal" aria-label="Close" onclick = '$(".conclave").fadeIn("slow"); $(".sinclave").fadeOut("slow"); $("#ingresaClaveCon").val(""); $("#controlExistentes").html("") ; $("#recibePruebas").html("") ; $("#cantidadExistente").html("") ' ><span aria-hidden="true">&times;</span></button>
												<div class="alert alert-info" role="alert">
													<span class="glyphicon glyphicon-alert" aria-hidden="true"></span>
													&nbsp;&nbsp;<strong>Alerta !!!</strong>&nbsp;Se reimprimirá el (los) boleto(s) seleccionados del usuario :  <?php echo $_SESSION['useractual'];?>
												</div>
											</div>
											<div class="modal-body">
												<div class = 'row' >
													...
													<div class = 'col-md-1 sinclave' style = 'display:none;'>
														Boleto Desde
														<input  type = 'text' class = 'form-control'  placeholder = 'DESDE SOLO NUMERO' id = 'desde' />
													</div>
													<div class = 'col-md-1 sinclave' style = 'display:none;'>
														Boleto Hasta
														<input  type = 'text' class = 'form-control'  placeholder = 'HASTA SOLO NUMERO' id = 'hasta' />
													</div>
													
													<div class = 'col-md-1 sinclave' style = 'display:none;'><br>
														<button type="button" class="btn btn-danger" id = 'verifica_boleto' onclick = 'verificarBoletos();'>VERIFICAR</button>
													</div>
													<div class = 'col-lg-12' id = 'recibeControlReimpresiones'>
														<div class = 'row'>
															<div class = 'col-md-3' id = 'cantidadExistente' ></div>
															<div class = 'col-md-6' id = 'construirCantidad' ></div>
															<div class = 'col-md-2' id = '' >
																<div id = 'controlExistentes'  style = 'display:none;'>
																	<button type="button" class="btn btn-info" onclick = 'enviaReimprimir()' >&nbsp;&nbsp;Grabar Reimpresiones&nbsp;&nbsp;</button>
																	<button type="button" class="btn btn-danger" onclick = "$('#construirCantidad').html('');$('#recibeIdBoletos').val('');$('#cantidadExistente').html('');" >Cancelar Reimpresiones</button>
																	<input type = 'hidden' id = 'recibeIdBoletos' />
																</div>
															</div>
														</div>
													</div>
													<div style = 'display:none;'><div class="col-md-1 sinclave" style="display: none;">
														<!--<button type="button" id="evento_baja" class="btn btn-warning" onclick = '$("#ver_baja_lote1").modal("show")'>VER DADOS DE BAJA</button>-->
														<h3><div id="bajas"></div><div id="showsum"></div></h3><h3><div id="bajasshow"></div></h3><input id="bajasx" type="hidden" name="" value=""><br><br>
													</div></div>
												</div>
											</div>
											<div class="modal-footer" >
								...
								<script>
									function enviaReimprimir(){
										...
										var servicesFormatted = $('#recibeIdBoletos').val();
										if(servicesFormatted == ''){
											alert('debe haber al menos un ticket asignado en el rango seleccionado')
										}else{
											$.post("subpages/reimprime_lote.php",{ 
												servicesFormatted : servicesFormatted , distribuidor : distribuidor
											}).done(function(data){
												alert(data);
												
												$('#construirCantidad').html('');$('#recibeIdBoletos').val('');$('#cantidadExistente').html('');
												$('#desde').val('');
												$('#hasta').val('');
												$('#recibePruebas').html('');
									...
									function verificarBoletos(){
										...
										if(desde == '' || hasta == '' || distribuidor == '' || localidad == ''){
											
										}else{
											if(parseInt(hasta) < parseInt(desde) ){
												alert('el boleto desde no puede ser mayor al boleto hasta');
												$('#desde').val('');
												$('#hasta').val('');
											}else{
												$('#verifica_boleto').attr('disabled',true);
												$('#verifica_boleto').html('Espere!!!');
												$.post("spadmin/verificarBoletos.php",{ 
													desde : desde , hasta : hasta , distribuidor : distribuidor , localidad : localidad ,  evento , evento
												}).done(function(data){
													
													$('#verifica_boleto').attr('disabled',false);
													$('#verifica_boleto').html('VERIFICAR');
												
													str = data.split('|');
													var cantidadExistente = str[0];
													var construirCantidad = str[1];
													var recibeIdBoletos = str[2];
													
													if(cantidadExistente > 0){
														
														
														$('#cantidadExistente').html('Atención, en el rango ingresado se a encontrado : <h1 style="margin:0px;padding:0px;text-align: center; color: red">' + cantidadExistente + '</h1> ticket(s) vendidos por este usuario');
														$('#construirCantidad').html('<table class = "table table-bordered">' +construirCantidad + '</table>');
														$('#recibeIdBoletos').val(recibeIdBoletos);
														$('#controlExistentes').fadeIn('fast');
														
														
														var altoCantidadExistente = $('#construirCantidad').height();
														// alert(altoCantidadExistente)	
														if(altoCantidadExistente >= 100){
															$('#construirCantidad').css('max-height','150px');
															$('#construirCantidad').css('overflow-y','scroll');
														}else{
															$('#construirCantidad').css('overflow-y','hidden');
														}
														
														
													}else{
														$('#cantidadExistente').html('<label style = "color:red;">Atención, en el rango ingresado no tiene ticket(s) vendidos por este usuario</label>');
														$('#construirCantidad').html('');
														$('#controlExistentes').fadeOut('fast');
													}
												});
											}
										}
				...
				<script>
					...
					function bajaDist() {
						...
						$.ajax({
							method:'POST',
							url:'spadmin/saberPventaEvento_.php',
