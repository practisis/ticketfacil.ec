<?php
	...
	echo "<input type='hidden' name='' value=".$pventa." id='paraprobar'>";
		...
		<tr>
			<td style = 'border-right:1px solid #000;'>
				<table class = 'table'>
					<tr>
						<th colspan = '9' >
							Detalle Ventas Baja Evento : <?php echo $rowCo['strEvento']."   [".$idcon."]";?>
						</th>
					</tr>
					<tr>
						<th>
			...
			<?php
				...
				while($row = mysql_fetch_array($res)){
					...
					while($rowL1 = mysql_fetch_array($resL1)){
						...
						$sqlBaja = 'SELECT count(idBoleto) as vendidos_todos FROM `Boleto` WHERE `idLocB` = "'.$rowL1['idLocalidad'].'" and tercera = 0 and id_desc != "'.$rowDD['id'].'" '.$query.' ORDER BY `idLocB` DESC';
						$resBaja = mysql_query($sqlBaja) or die (mysql_error());
						$rowBaja = mysql_fetch_array($resBaja);
			...
			?>
					<tr class = 'valores_macros' >
						...
						<td>
							<?php 
								/*$sqlBaja = 'SELECT count(idBoleto) as vendidos_normal FROM `Boleto` WHERE `idCon` = "'.$idcon.'0000" AND `idLocB` = "'.$row['idLocalidad'].'" AND `tercera` = 0 AND `id_usu_venta` = '.$pventa.' ORDER BY `idLocB` DESC';
								$resBaja = mysql_query($sqlBaja) or die (mysql_error());
								$rowBaja = mysql_fetch_array($resBaja);*/


								if($rowBaja['vendidos_normal'] == 0){
									$colorT2 = '';
								}else{
									$colorT2='background-color: #000;text-align: center; color:green;font-weight: bold;font-size: 15px';
								}
								echo "<h4 style = '".$colorT2."' >".$rowBaja['vendidos_normal']."<h4>";
							?>
						</td>
