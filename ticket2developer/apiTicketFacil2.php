<?php
	...
	if($id == 20){
		...
		$json ='{ "tabla": { "boletos" : ['	;
			$query = "SELECT idBoleto, strQr, strBarcode, idCli, idCon, idLocB, nombreHISB, documentoHISB, strEstado, strDescripcionL FROM Boleto JOIN Localidad ON Boleto.idLocB = Localidad.idLocalidad WHERE strQR = '$codigo' OR strBarcode = '$codigo'" or die(mysqli_error($mysqli));
			$result = $mysqli->query($query);
			while($row = mysqli_fetch_array($result)){
